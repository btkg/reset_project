...
string operator*(string s1, string s2)
{
    ...
    for (int i = s2.length() - 1; i >= 0; i--)
    {
        ...
        case '0':
            break;
        case '1':
            result = result + ((temp = prod1).append(end));
            break;
        case '2':
            result = result + ((temp = prod2).append(end));
            break;
        case '3':
            result = result + ((temp = prod3).append(end));
            break;
        case '4':
            result = result + ((temp = prod4).append(end));
            break;
        case '5':
            result = result + ((temp = prod5).append(end));
            break;
        case '6':
            result = result + ((temp = prod6).append(end));
            break;
        case '7':
            result = result + ((temp = prod7).append(end));
            break;
        case '8':
            result = result + ((temp = prod8).append(end));
            break;
        case '9':
            result = result + ((temp = prod9).append(end));
            break;
        }
    }
    if (result != "0")
...
string operator/(string s1, string s2)
{
    ...
    else //s1>s2
    {
        ...
        while (i <= s1.length()) //没超出s1的范围
        {
            ...
            if (devidend.find_first_not_of('0') == string::npos)
                devidend = "0";
            else
                devidend = devidend.substr(devidend.find_first_not_of('0'), string::npos);
            i++;
        }
    }
    if (result != "0")
